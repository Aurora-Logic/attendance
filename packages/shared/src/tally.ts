...
export interface PayrollJournalInput {
  ...
  /**
   * Run version. It belongs in the voucher number: a correction run for the
   * same month otherwise carries the identical number, so Tally either rejects
   * it as a duplicate or books the month's salary twice.
   */
  version?: number
}
...
export function buildPayrollJournal(input: PayrollJournalInput): string {
  ...
  return buildTallyVoucherXml({
    ...
    voucherType: "Journal",
    voucherNumber: input.version && input.version > 1
      ? `PAY/${input.month}/V${input.version}`
      : `PAY/${input.month}`,
    dateISO,
