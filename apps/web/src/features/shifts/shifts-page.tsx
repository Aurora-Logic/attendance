...
import { DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE } from '@vyuha/shared';

import { ShiftSheet } from './shift-sheet';
import type { RosterEntry, Shift } from './types';
import { useRoster, useShifts } from './use-shifts';

/**
 * REQ-C-01…C-05 / PRD §5 screen 11: shift masters and who is on them.
 *
 * Two tabs rather than two routes. A roster is meaningless without the shift
 * definitions beside it — "Night, 22:00–06:00" is the only thing that makes a
 * roster row readable — and splitting them would make checking one against the
 * other a navigation.
 */
...
const SHIFT_COLUMNS: RecordColumn<Shift>[] = [
  ...
  },
  {
    ...
    header: 'Midnight',
    cell: (row) =>
      row.crossesMidnight ? <Badge variant="secondary">Crosses</Badge> : EMPTY_VALUE,
  },
];
...
export function ShiftsPage() {
  return (
    <>
      <PageHeader description="Shift definitions and who is assigned to them." />
