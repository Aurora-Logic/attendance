...
export class PushQueueService {
  ...
  /** The connection an agent can work for this organisation, or null. */
  async agentConnection(orgId: string): Promise<string | null> {
  ...
  async enqueue(orgId: string, actorUserId: string | null, payload: VoucherPushPayload): Promise<string | null> {
    const connectionId = await this.agentConnection(orgId);
