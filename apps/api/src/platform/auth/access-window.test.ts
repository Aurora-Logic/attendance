...
describe('the access window (12 Area AB)', () => {
  it('is open in the working day, closed after 19:30 and before 09:00, on the organisation clock', () => {
    ...
    expect(evaluateWindow(window, IST, at('2026-08-18T14:30:00Z'))).toEqual({ closed: true, reopensAt: '09:00 tomorrow', closesInMinutes: null });
    expect(evaluateWindow(window, IST, at('2026-08-19T02:00:00Z'))).toEqual({ closed: true, reopensAt: '09:00', closesInMinutes: null }); // 07:30 IST Wed
  ...
  it('REQ-AB-05: while open on an applicable day it says how long until the close, so a client can warn at 19:15', () => {
    // 13:45 UTC = 19:15 IST Tuesday: fifteen minutes to go.
    expect(evaluateWindow(window, IST, at('2026-08-18T13:45:00Z'))).toEqual({ closed: false, reopensAt: null, closesInMinutes: 15 });
    // Sunday does not apply: nothing to warn about.
    expect(evaluateWindow(window, IST, at('2026-08-23T13:45:00Z')).closesInMinutes).toBeNull();
  });
