...
| P-29 | Customer mails E1–E4 | E1 shipped and E2 delivered go by themselves from the organisation's mailbox (HTML + text, barcode as table cells, reply-to the profile's email), outcome recorded on the notice row, never failing a dispatch; renderer unit-tested. **E3 ready-to-collect and E4 invoice are not built**: E3 needs a customer-collects dispatch mode the product does not have, E4 needs a server-side PDF that D-46 ruled out (browser print). Both wait on a decision | Platform | E1/E2 done; E3/E4 need a decision | Partly done |
