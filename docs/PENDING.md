...
| P-04 | Comparison control | Year·Quarter·Month + off/vs-previous/vs-last-FY, party scope, Indian FY, like-for-like partial periods, URL state, deltas in charts/tables/KPIs/exports | Reports | No (spec is explicit) | Not started |
...
| P-06 | Dual view on report pages | Table·Chart·Both toggle per report per device; chart over the **full filtered set**, not the page; chart drill applies filter and switches to table | Reports | No | In progress — toggle + charts live off the current page (≤200 rows); full-set aggregation + chart-drill pending |
